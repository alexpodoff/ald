...
import pytest
from .ald_admin_data import mac_cat_invalid_generator, valid_maclev_generator

__author__ = 'pod'
__version__ = '0.1'


@pytest.mark.parametrize('cat', mac_cat_invalid_generator())
def test_user_maclev_mod(ald_init, cat, app):
    admpass = ald_init[0]
    passfile = ald_init[1]
    ...
    if hex(min_cat) != '0x0' and hex(min_cat) not in maccat_list:
        ...
        maccat_list = app.macattr.maccat_list()
    if hex(max_cat) != '0x0' and hex(max_cat) not in maccat_list:
    ...
    assert '(%s)' % str(hex(min_cat)) not in user_full_info
    if max_cat != 0x0:
        assert '(%s)' % str(hex(max_cat)) not in user_full_info
