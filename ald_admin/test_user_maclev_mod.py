...
import pytest
from .ald_admin_data import mac_level_combinations_generator, valid_maclev_generator

__author__ = 'pod'
__version__ = '0.1'


@pytest.mark.parametrize('level', mac_level_combinations_generator())
def test_user_maclev_mod(ald_init, level, app):
    ...
    assert '(%s)' % str(min_level) in user_full_info
    assert '(%s)' % str(max_level) in user_full_info
